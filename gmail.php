...
foreach ($accounts as $accountIndex => $account) {
    ...
    foreach ($remainingTargets as $targetIndex => $recipient) {
        ...
        if ($authFailures >= $maxAuthFailures) {
            echo "Reached authentication failure threshold for $email\n";
            break;
        }
        ...
        try {
            ...
            if (
                ...
                stripos($error, 'sending limit') !== false ||
                stripos($error, '5.4.5') !== false
            ) {
                $quotaFailures++;
                echo "Quota error $quotaFailures for $email\n";

                if ($quotaFailures >= $maxQuotaFailures) {
                    echo "Skipping account due to quota limits.\n";
                    break;
                }
            }

            if (
                stripos($error, 'Could not authenticate') !== false ||
                stripos($error, 'authentication failed') !== false ||
                stripos($error, 'QUIT command failed') !== false
            ) {
                $authFailures++;
                echo "Authentication failure $authFailures for $email\n";

                if ($authFailures >= $maxAuthFailures) {
                    echo "Skipping account due to repeated authentication failures.\n";
                    break;
                }
            }
